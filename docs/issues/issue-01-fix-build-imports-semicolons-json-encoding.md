# Fix build: imports, semicolons, JSON encoding

## Target
All code under `src/sacrum_gleam/` and `src/sacrum_gleam.gleam`
...
## Problem
`gleam build` fails due to:
1. **Semicolons**: Gleam 1.14 removed semicolons as whitespace — must eliminate them
2. **Obsolete imports**: `import gleam/json` references removed but usages remain
3. **Manual JSON encoding**: replace `json.array(...)` with custom function
4. **Detached doc comments**: in `db/flows.gleam`

## Change
1. Run `gleam build`, read ALL errors
2. Remove all `;` in inline case expressions
3. Remove any remaining `import gleam/json`
4. Replace `json.*` calls with manual helpers
5. Fix detached doc comments in `db/flows.gleam`
6. Verify clean `gleam build` (warnings acceptable)

## Acceptance
- `gleam build` compiles without errors
- `gleam test` passes (even gleeunit scaffold test)
- No business logic changes, syntax only
